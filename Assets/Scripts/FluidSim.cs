...
public class FluidSim : MonoBehaviour
{
	...
	public void ScaleDensity(float factor)
	{
		for (int i = 0; i < size; i++)
		{
			dens[i] *= factor;
	...
	void vel_step(float[] u, float[] v, float[] u0, float[] v0, float dt)
        //Velocity Step
	{
		float[] temp; //Temp array. Cycles between u0 and u, also cycles between v0 and v
		addFields(u, u0, dt); 
		addFields(v, v0, dt);
		temp = u0; u0 = u; u = temp;
		temp = v0; v0 = v; v = temp;
	...
	{
		// draw on the water
		var mBtnLeft = Input.GetMouseButton(0);
		var mBtnRight = Input.GetMouseButton(1);
		if (mBtnLeft || mBtnRight) {
			Ray ray = Camera.main.ScreenPointToRay(Input.mousePosition);
			RaycastHit hit;
			if (Physics.Raycast(ray, out hit, 100)) {
				// determine indices where the user clicked
				var vec = WorldTo2DIndex(hit.point);
				var i = WorldToIndex(hit.point);
				
				if (vec.x < 1 || vec.x > width-1 || vec.y < 1 || vec.y > height-1) return;
				// add or dec density
	...
	{
		var vec = WorldTo2DIndex(pos);
		int i = (vec.x + 1) + (vec.y + 1) * rowSize;
		return i;
	}

	Vector2Int WorldTo2DIndex(Vector3 pos)
	{
		int x = (int)(pos.x * width);
		int y = (int)(pos.z * width);
        if (x < 1 || x > width - 1 || y < 1 || y > height - 1) return new Vector2Int(0, 0);
        return new Vector2Int(x,y);		
	}

    public void FlowSourceAtPos(Vector3 worldPos, Vector2 flowStrength)
    {
	    var i = WorldToIndex(worldPos);
	    
	    dens_prev[i] += 3f;
	    // add velocity
	    u_prev[i] += flowStrength.x;
	    v_prev[i] += flowStrength.y;	    
    }

	public Vector3 GetVelocityAtPos(Vector3 worldPos)
	{
		var i = WorldToIndex(worldPos);
		
		return new Vector3(u[i], 0, v[i]);		
	}

	public float GetDensityAtPos(Vector3 worldPos)
	{
		var i = WorldToIndex(worldPos);
		return dens[i];
	}
